...
for n in range(5,50,8):
    ...
    s = time.time()
    best_state, best_fitness, fitness_curve = ml.random_hill_climb(problem, max_attempts=100, max_iters=4000,
                                                                   restarts=8,
                                                                    curve=True)
    e = time.time()
    t = e - s
    ...
    s2 = time.time()
    best_state2, best_fitness2, fitness_curve2 = ml.genetic_alg(problem, pop_size=160, mutation_prob=.1,
...
plt.legend()
plt.xlabel('size of problem')
plt.ylabel('# of function eval.')
plt.title('# of function evaluations (knapsack)')
